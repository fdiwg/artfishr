...
The R package `artfishr` implements tools for producing catch and effort estimates from sample-based survey data of small-scales fisheries data following the *FAO Artfish extrapolation methodology*.  
...
1. **Fisher interviews (`effort_source = "fisher_interview"`)**  
   ...
   - The template x is required (number of days fished within the period of reference)
   - In that case, the active days survey is not necessary. Total number of days in the month is used by default.
  

2. **Boat counting (`effort_source = "boat_counting"`)**  
   - Data collected via registers or direct observation
   - Template y is required (number of boats out in comparison of total potentially active boat)

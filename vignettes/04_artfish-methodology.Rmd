...
- **Catch** refers to the total catch (all species combined) and is usually estimated within a defined context:
...
```{r fig-artfish-concept, echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("figures/artfish_schema.png")
```
**Figure 1.** ARTFISH conceptual framework.
...
Fishing unit is the minimal stratification that must be definied.

> **Important**
>
> Population parameters are always estimated at the lowest level of stratification (by stratum, so by year/month/(major straum)/(minor stratum)/fishing unit). Estimates at higher levels are obtained by aggregating the corresponding lower-level estimates.
...
- 8 to 12 sampling days per month homogeously distributed (if possible week and weenkend days);
...
```{r fig-cpue-principle, echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("figures/artfish_CPUE_schema.png")
```
...
- $UnitEffort_i$ is the fishing effort associated with the $i$-th sampled fishing trip.
- $n$ is the number of fishing trips sampled within the estimation context.
...
```{r fig-effort-principle, echo=FALSE, out.width="80%", fig.align="center"}
knitr::include_graphics("figures/artfish_effort_schema.png")
```
...
- $D$ is the raising factor expressing the total number of days with fishing activity during the reference period. It is estimated from an active days survey.
- $F$ is the raising factor expressing the total number of fishing units potentially operating across all fishing sites within the stratum. It is estimated from a frame survey or a vessel registry.
- $AC$ is the activity coefficient. There are two main ways to calculate the activity coefficient that will impact the design of the survey. Fishing unit activity can be estimated either through the boat activity coefficient ($BAC$), using vertical sampling (boat counts), or through the fishing activity coefficient ($FAC$), using horizontal sampling (fisher interviews).
...
Both $BAC$ and $FAC$ express the probability that a fishing unit (boat) is active on any given day during the reference period. $BAC$ is estimated from a boat activity survey, whereas $FAC$ is estimated from fisher interviews.

### 4.2 Number of active vessels: Estimation of the raising factor expressing the total number of active fishing units ($F$)

The raising factor $F$ represents the number of boats that are active within the fishing fleet and are therefore potentially capable of fishing during the sampling period.
...
For stratum $i$ and fishing unit $j$, $F$ is calculated as the sum of the number of boats belonging to fishing unit $j$ across all sites included in the stratum:
...
- $x_{i,j,k}$ is the number of potentially active vessels of fishing unit $j$ at site $k$ within stratum $i$.
- $k$ is the site index, ranging from $1$ to $q$. *(to complete if required)*
- $q$ is the total number of sites within stratum $i$.
- $F_{i,j}$ is the raising factor for stratum $i$ and fishing unit $j$.

### 4.3 Number of active days: Estimation of the time raising factor ($D$)

The time raising factor $D$ represents the total number of days during the reference period that are considered normal fishing days. It is obtained by starting from the total number of calendar days and subtracting days during which fishing is not expected to occur (e.g. weekends, public holidays, or bad weather).

The value of $D$ is estimated for a given estimation context (reference period, geographical stratum, and fishing unit). The target population therefore corresponds to all calendar days within the reference period.
...
When several sites belong to the same stratum, $D$ is calculated as the weighted mean of the site-specific values, using the number of boats as weights.
...
- $x_{i,j,m}$ is the number of active boats at site $m$ in stratum $i$ for fishing unit $j$.
- $D_{i,j,m}$ is the estimated number of active fishing days at site $m$ in stratum $i$ for fishing unit $j$.
...
Fishing activity can be estimated either through the boat activity coefficient ($BAC$), based on vertical sampling, or through the fishing activity coefficient ($FAC$), based on horizontal sampling. These two options are possible, but the fisher interview is easier to implement (as it can be include in the landing survey) and usually gives more reliable estimates.
...
- **Fishing activity coefficient (fisher interview)** Information on fishing activity is collected by interviewing fishers during the landing survey. A typical question asks how many days they have fished during a fixed reference period (e.g. the previous 5 or 30 days). This approach is used to estimate $FAC$. In case of fisher interview, the active days survey is not necessary, by default the total number of days in the moth is used.

- **Boat activity coefficient (boat counting)** Fishing activity is estimated by counting, at regular intervals, the total number of boats present at landing sites and the number of boats that actually went fishing. This approach is more demanding but may be preferable where boat movements are important or fishing activity is influenced by seasonal or lunar cycles (e.g. light fishing for sardinellas). This approach is used to estimate $BAC$. The boat counting must be compemented by the active days survey (or at least an estimation of tha active days per month) for each stratum.

#### 4.4.2 Boat activity coefficient

For stratum $i$ and fishing unit $j$, $BAC$ is calculated by dividing the total number of boats that went fishing by the total number of boats observed (from either the frame survey or the vessel registry):
...
- $Boats_{i,j,m,e}$ is the number of boats recorded at site $m$ in stratum $i$ for fishing unit $j$.
- $BoatsOut_{i,j,m,e}$ is the number of boats that went fishing on day $e$ at site $m$ in stratum $i$ for fishing unit $j$.
...
- $x_{i,j,m}$ is the number of boats at site $m$ in stratum $i$ for fishing unit $j$.
...
- $DaysActive_{i,j,b}$ is the number of days the interviewed fisher was active during the reference period.
- $RefTime$ is the duration of the reference period (e.g. 5 or 30 days).
...
- $SpeciesCatch$ is the estimated catch of a given species within the estimation context.
- $SP$ is the proportion of the total catch represented by the species in the sampled landings.
...
- $i$ is the stratum index, corresponding to a unique combination of reference period, geographical stratum, and fishing unit.
...
The methodology combines algebraic and probabilistic approaches to evaluate the accuracy of both CPUE and activity coefficient ($AC$) estimates.

The following indicators are produced:

- Spatial accuracy of the activity coefficient ($AC$);
- Temporal accuracy of the activity coefficient ($AC$), which is always equal to 1 and therefore does not require estimation;

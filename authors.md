# Authors and Citation

## Authors

- **Emmanuel Blondel**. Author, maintainer.
  [](https://orcid.org/0000-0002-5870-5762)

- **Alexandre Bennici**. Author.
  [](https://orcid.org/0000-0003-2160-3487)

- **Johanna Herfaut**. Author.

## Citation

Source:
[`DESCRIPTION`](https://github.com/fdiwg/artfishr/blob/HEAD/DESCRIPTION)

Blondel E, Bennici A, Herfaut J (2026). *artfishr: ArtFish for R*. R
package version 0.1.20260415, <https://github.com/fdiwg/artfishr>.

    @Manual{,
      title = {artfishr: ArtFish for R},
      author = {Emmanuel Blondel and Alexandre Bennici and Johanna Herfaut},
      year = {2026},
      note = {R package version 0.1.20260415},
      url = {https://github.com/fdiwg/artfishr},
    }

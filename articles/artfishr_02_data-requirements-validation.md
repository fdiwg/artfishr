...
| Effort (boat counting) | `inst/extdata/format_specs/artfish_B2_effort.json` | `inst/extdata/samples/effort_boat_couting.csv` |
...
chosen during the data collection design.
...
coefficient which the probability that a boat have been at sea on a
certain day.
...
1.  **Fisher interviews (`effort_source = "fisher_interview"`)**
    ...
    - In this case, the active days survey is not necessary. Total
      ...
      already included this component. active_day is NULL then and will
...
2.  **Boat counting (`effort_source = "boat_counting"`)**
    ...
    - Template `effort_boat_counting.csv` is required (number of boats
      ...
      to the tab B2. Effort (boat counting) of the Excel template
...
# Load sample datasets
active_vessels <- readr::read_csv(system.file("extdata/samples/active_vessels.csv", package = "artfishr"))
effort <- readr::read_csv(system.file("extdata/samples/effort_boat_counting.csv", package = "artfishr"))
landings <- readr::read_csv(system.file("extdata/samples/landings.csv", package = "artfishr"))
